...
class WarWorld(World):
    def __init__(self):
        super(WarWorld, self).__init__()
        # War world has multipe agents
        self.enemy = Agent()
        self.ranger = Agent()
        self.uav = Agent()
        self.agents = [self.enemy, self.ranger, self.uav]
        for i, agent in enumerate(self.agents):
            agent.id = i
            agent.name = ['enemy', 'ranger', 'uav'][i]
    ...
    def step(self):
        ...
        # check if enemy caught
        self.was_enemy_caught = False
        if self.is_collision(self.ranger, self.enemy) and not self.enemy.caught:
            self.enemy.caught = True
            self.was_enemy_caught = True

        # check if any animal caught
        self.was_animal_caught = False
        for i, landmark in enumerate(self.landmarks):
            if self.is_collision(self.enemy, landmark):
                landmark.enemy_collision_time += 1
            else:
                landmark.enemy_collision_time = 0

            if landmark.enemy_collision_time >= 20 and not landmark.caught:
...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for i, landmark in enumerate(world.landmarks):
            ...
            landmark.enemy_collision_time = 0
            landmark.caught = False
        world.was_enemy_caught = False # has the enemy been caught
    ...
    def done(self, agent, world):
        ...
        '''
        if world.was_enemy_caught or world.was_animal_caught:
            return 1
        else:
            return 0
    ...
    def reward(self, agent, world):
        ...
        '''
        if world.was_enemy_caught:
            return agent.enemy_caught_rew
    ...
    def is_observed(self, agent, target, world):
        ...
        if agent.name == 'enemy': # enemy -> ranger/uav
            return self.is_visible(agent, target)
        elif target.name == 'enemy': # ranger/uav -> enemy 
